"""
Оркестратор библиотеки: класс Replicator и проверки его параметров.

Читает изменения из 1С через план обмена (ChangeReader), пишет их в целевую БД (DBWriter) и
подтверждает приём пакета только после успешного сохранения. Он же исполняет полную выгрузку —
чтение объекта целиком, страницами, с пометкой строк, которых в 1С больше нет.

Всё, что видно пользователю, — здесь; читатели, писатель и метаданные вызываются отсюда.
"""
...
class Replicator:
    ...
    def _deliver_signal(self, conn, table_name: str, source: str | None, result,
        ...
        Сообщаем через БД: ставим метку update_requested_at в onecdc_handlers тем, у кого эта таблица
    ...
    def full_load(self, object_name: str, batch_size: int = 1000,
        ...
        """
        Читает объект 1С целиком и сводит с тем, что уже лежит в БД. Идемпотентно: повторный
        прогон обновляет строки по ключу, а вложенные табличные части приезжают вместе с владельцем.

        Возвращает число РЕАЛЬНО изменённых строк по всем страницам. Это проверка самого CDC:
        если изменения доезжают исправно, выгрузке менять нечего и ответ должен быть 0.

        Страницы. batch_size — верхняя граница, а не размер: реальный подбирается по весу ответа
        (_next_page_size), а не осиленная 1С страница повторяется меньшей. Листаем через $skip —
        курсора по ключу 1С не даёт (DESIGN.md, «Почему страницы берутся только через $skip»), и
        глубокий объект поэтому перечитывается окнами по периоду (_load_by_windows).

        Гонка с изменениями. Каждая страница сохраняется со СВОЕЙ отметкой — не «сейчас», а
        границей по реестру незавершённых merge (см. _load_pages). Всё, что переписано позже
        отметки, снимок не трогает; всё, что старше, перезаписывает. См. DBWriter.save.

        mark_missing (по умолчанию ДА) помечает строки, которых 1С не отдала: физическое удаление
        объекта в обмен не приходит вовсе, и без пометки такая строка остаётся навсегда, а витрина
        не заметит её исчезновения — она смотрит на merged_on. Помечается область, которую прогон
        читал (выгрузка за период — только свой период), и помечается без переспроса: не увидели —
        значит помечаем, ложная пометка снимется ближайшим изменением (DESIGN.md, «Не увидели —
        помечаем»). Плата — таблица ключей на прогон и один UPDATE в конце.

        date_field/date_from/date_to — ручная догрузка за период; границы включительно, поле даты
        зовётся как в 1С или как в БД. read_subconto=None означает «как задано у репликатора»;
        по умолчанию субконто не читаются — адресовать их виртуальную таблицу можно только
        списками периодов, то есть многими запросами (DataReader._fill_subconto).

        Объект занимается на время выгрузки, поэтому двойной работы не будет, даже если его же
        читает фоновый воркер или такой же скрипт в соседнем контейнере. Занят — по умолчанию
        пропускаем (WARNING и 0 в ответе: мы ничего не изменили). Кому надо отличать это от
        «выгрузили, расхождений нет» — skip_if_busy=False и FullLoadBusy; так делают наши же
        воркер и расписание, которым нельзя отметить объект выгруженным, не выгрузив его.

        Один прогон = одна строка в onecdc_replicator_log; finished_at ставится после всех страниц.
        """
        if not self.metadata.is_loaded:
            self.metadata.get_metadata()
    ...
    def _mark_orphaned_table_parts(self, object_name: str, target, started_at,
        ...
        Идёт ПОСЛЕ пометки владельца: не помеченный владелец своих частей не тянет, поэтому
        решение принимается один раз и в одном месте.
    ...
    def _marking_scope(target, date_column: str | None,
        ...
        встреченная в прогоне, а прогон видел только своё окно. Переспросить 1С про каждого
        кандидата было бы вторым способом, но его нет — см. DESIGN.md, «Не увидели — помечаем».
    ...
    def run_forever(self, interval: float = 60.0, max_iterations: int = 0,
        ...
        потоков полной выгрузки, два потока отметки живости (незавершённых merge и захвата полной
        выгрузки) и по потоку на каждого обработчика. Обработчики в пул выгрузки не сабмитятся и
        занять его не могут. Дефицит возникает не в потоках, а в соединениях общего engine:
        одновременно их держат пакет изменений, страницы выгрузки и каждый обработчик, отсюда
        pool_size + max_overflow >= full_load_workers + 1 + число обработчиков + число расписаний
        FullLoadCron (см. README_DB.md, «Сколько нужно соединений к БД»). Отметки живости в этот
        счёт не входят — у них свой маленький пул, как раз чтобы страницы выгрузки не оставили их
        без соединения: не продлённая дольше своего TTL отметка неотличима от мёртвого процесса.
        """
