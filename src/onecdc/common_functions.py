"""
Мелкие общие помощники: то, что нужно нескольким модулям и не тянет на свой.

Guid'ы и имена объектов, заголовок авторизации OData (свой, потому что Basic из requests кодируется
в latin-1 и падает на кириллице), разбор ответа 1С и извлечение из него текста ошибки, чтение
ответа с потолком размера, форматирование байт и длительностей, имя экземпляра процесса.
"""
...
# Часы БД. Момент, а не показание настенных часов: служебные отметки лежат в timestamptz, и
# приводить now() больше не к чему — драйвер отдаёт offset-aware datetime, колонка хранит его как
# есть, сравнения идут aware с aware.
#
# Раньше здесь стояло `func.now().cast(DateTime)`, и вся служебная разметка времени была наивной,
# то есть настенной. Настенное время немонотонно: в поясе с сезонным переводом осенний час
# проживается дважды, и час инкремента терялся молча — merged_on строк этого часа оказывался ниже
# last_run_at (CDC-25). Двусмысленность наивного значения в повторяющийся час неустранима по
# существу, поэтому лечится она типом, а не аккуратностью сравнений.
#
# Даты, приехавшие из 1С (Период, Дата), остаются НАИВНЫМИ: в 1С поясов нет, это бизнес-значения,
# а не моменты. Граница между двумя видами времени проходит здесь и закреплена в
# replicator._check_period_bound.
DB_NOW_WITH_TIMEZONE = func.now()
...
def odata_datetime_value(value: "date | datetime") -> str:
    ...
    прочитанных из самой 1С (курсор по периоду, границы окна). Годы меньше 1000 встречаются и как
    опечатка оператора.
