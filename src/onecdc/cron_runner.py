...
class FullLoadCron:
    ...
    прогон читал: при заданном периоде — только строки этого периода. Переспрашивать 1С про
    каждого кандидата мы не пробуем — почему, в DESIGN.md, «Не увидели — помечаем».
...
def _bound(bound: DateBound):
    """
    Граница периода для full_load: `timedelta` — смещение назад, остальное как передали.

    От чего смещение, зависит от самого смещения, и это не прихоть. Целые сутки означают «N дней
    назад» — считаем от сегодняшней ДАТЫ, то есть от полуночи, как и раньше: ночное расписание
    «перечитать последнюю неделю» должно брать неделю целиком, а не с текущего часа.

    А смещение меньше суток (или с остатком) от даты отсчитать нельзя — `date.today() - bound`
    отбрасывает часы молча: `timedelta(hours=12)` давал СЕГОДНЯ, то есть окно «полсуток назад»
    оказывалось пустым. Такое смещение считаем от текущего МОМЕНТА — единственное прочтение, при
    котором часы не теряются.
    """
    if isinstance(bound, timedelta):
        if bound % timedelta(days=1):
            return (datetime.now() - bound).replace(microsecond=0)
