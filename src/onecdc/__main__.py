...
def main() -> None:
    ...
    # Потолок соединений — это СУММА pool_size и max_overflow (по умолчанию он 10, и тогда
    # pool_size=5 на деле удерживает 15), поэтому задаём оба (см. README_DB.md, «Сколько нужно
    # соединений к БД»). В pool_size — непрерывная работа: цикл изменений и запас на его попутные
    # запросы. Страницы выгрузки идут через overflow: такое соединение закрывается сразу, как
    # отработало, а не висит простаивая. Отметки живости сюда не входят — у них свой пул.
