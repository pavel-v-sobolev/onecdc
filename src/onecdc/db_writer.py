"""
Запись прочитанного в целевую БД: DBWriter.

Один вызов save() — один объект 1С: таблица создаётся или расширяется по метаданным, строки
сливаются через dbmerge, исчезнувшие из набора помечаются. Имена таблиц и колонок переводит
NameMapper, типы и первичный ключ приходят из метаданных.
"""
