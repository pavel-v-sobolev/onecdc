...
Репликатор обработчиков не запускает и о них не знает: сигналы идут через onecdc_handlers.
...
# Непрерывно работает одно соединение — цикл чтения изменений. Ещё два держим запасом: на его же
# попутную работу (журнал, метаданные, захват объекта) и на случай, если отдельный пул под отметки
# живости не поднялся и они пошли сюда.
REPLICATOR_CONNECTIONS = 3

# Пул считается, а не подбирается на глаз: на каждый обработчик и каждое расписание нужно своё
# соединение — они работают своими потоками (подробнее в README_DB.md, «Сколько нужно соединений»).
# Потолок соединений — это СУММА pool_size и max_overflow, и задавать её надо самим: по
# умолчанию max_overflow=10, то есть pool_size=5 на деле удерживает 15 (измерено), а три
# контейнера дают 45 против лимита сервера.
#
# Делим так: в pool_size — то, что работает непрерывно; страницы выгрузки, витрины и расписания
# идут через overflow. Разница в том, что соединение из
# pool_size, однажды открывшись, остаётся висеть, а overflow закрывается сразу, как отработало, —
# и простаивающих соединений мы держим три, а не пятнадцать. Переоткрытие стоит ~20 мс, на фоне
# страницы выгрузки незаметно.
#
# pool_pre_ping=True — соединение после простоя (ночь, перезапуск СУБД, failover) сервер уже
# закрыл, и первый же запрос по нему падает с «server closed the connection unexpectedly».
# Проверка при выдаче стоит 0.2 мс (измерено).
engine = create_engine(
    os.environ["ONECDC_DB_URL"],
    pool_size=REPLICATOR_CONNECTIONS,
    max_overflow=FULL_LOAD_WORKERS + len(HANDLERS) + len(CRON_JOBS),
    pool_pre_ping=True,
)
