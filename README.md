...
Таблица целиком потому, что переименование одной колонки ключа оставило бы таблицу нерабочей: в
Postgres колонка остаётся в первичном ключе и после переименования, а ключ не допускает пустых
значений — новые строки перестали бы вставляться вовсе. Да и сопоставить старые строки с
источником после смены типа ключа уже нечем.
...
Отставленное не удаляется: и данные, и имя остаются на месте. Уберёте вы их сами, когда убедитесь,
что ничего не потеряли. Каждый такой случай видно в логе — с именем, под которым теперь лежит
старое.

Если тип колонки задали вы сами (например сузили `varchar` до `varchar(50)` или указали точность
`numeric`), это сменой типа не считается и колонку никто не тронет.
