...
## Переезд служебных отметок на timestamptz — СДЕЛАНО (см. CHANGELOG, Breaking)

**Зачем.** Служебные отметки сейчас `timestamp without time zone`: в БД лежит настенное время
пояса сессии, а не момент. В поясе с сезонным переводом настенные часы раз в год отступают на час
(измерено: под `Europe/Berlin` `now()::timestamp` идёт 02:59 → 02:05), и час инкремента теряется
молча — `merged_on` строк этого часа оказывается ниже `last_run_at`, и под `merged_on > last_run_at`
они не попадут уже никогда. Это CDC-25 аудита. Наивное значение в повторяющийся час
двусмысленно **по существу**, поэтому чинится оно только типом, а не аккуратностью сравнений.

**Поправка по ходу работы.** План писался в расчёте «данные не трогаем», но решено было
сделать единообразно, и это вскрыло то, чего в плане не было: PostgreSQL не даёт менять тип
колонки, на которую смотрит вьюшка, — а витрины строятся именно вьюшками. Поэтому миграция
снимает зависимые вьюшки и ставит обратно вместе с владельцем, правами и комментарием, одной
транзакцией. Таблицу под материализованным представлением не трогает вовсе.

**Что переводим:**

| таблица | колонки |
|---|---|
| `onecdc_handlers` | `last_run_at`, `update_requested_at`, `last_full_rebuild_dt`, `lease_heartbeat_at` |
| `onecdc_writes_in_process` | `started_at`, `heartbeat_at` |
| `onecdc_replicator_log` | `started_at`, `finished_at` |
| `onecdc_metadata_objects` | `full_load_heartbeat_at`, `last_full_load_dt`, `merged_on` |
| `onecdc_name_claims` | `claimed_at` |
| `onecdc_exchange_nodes` | `reader_heartbeat_at` |
| таблицы данных и витрин | **только** `merged_on` и `inserted_on` |

**Чего НЕ трогаем:** даты, приехавшие из 1С (`Период`, `Дата`, `ДатаОтгрузки`, …). В 1С поясов
нет; это бизнес-значения, а не моменты, и истолковать их в поясе сервера значило бы поменять их
смысл и поставить его в зависимость от настройки БД. Граница между двумя видами времени проходит
ровно здесь, и она же уже закреплена в `_check_period_bound`: границы полной выгрузки — наивные,
потому что это время 1С.

**Миграция — автоматическая, при старте.**

- Обнаружение по `pg_catalog` (не `information_schema`): нужен фильтр `relkind IN ('r','p')`, иначе
  в выборку попадают колонки ВЬЮШЕК, у которых тип не меняют, — измерено, в демо-схеме таких две.
  Стоимость проверки: 2.6 мс на 53 таблицы, 4.5 мс на 500. От объёма данных не зависит, поэтому
  остаётся в библиотеке навсегда, а не как разовый шаг: таблицы заводятся лениво, и созданная
  старой версией (или поднятая из бэкапа) иначе осталась бы наивной.
- Преобразование: `ALTER TABLE … ALTER COLUMN … TYPE timestamptz` **без `USING`** — PostgreSQL
  истолковывает старое наивное значение в поясе сессии, то есть ровно так, как оно записывалось;
  момент сохраняется, разрыва в данных нет.
- Цена: под не-UTC это полная перезапись таблицы под `ACCESS EXCLUSIVE`, ~0.9 с на миллион строк
  (PG 18, SSD, один индекс). Каждую таблицу — своей транзакцией, с именем и временем в логе.
- Идемпотентно и без гонок по построению: `ALTER` на уже переведённой колонке — пустая операция,
  второй процесс дождётся блокировки и увидит готовый тип (тот же приём, что в `_create_if_absent`).
- Служебные таблицы чинит `create_table_if_absent` — он уже сверяет состав колонок; добавляется
  сверка ТИПА объявленных datetime-колонок. Список имён при этом не нужен: у каждой таблицы есть
  своё объявление.

**Что меняется в коде:**

1. `DB_NOW_WITHOUT_TIMEZONE` → `DB_NOW_WITH_TIMEZONE` (просто `func.now()`, приведение уходит).
2. Все объявления служебных колонок — `DateTime(timezone=True)`.
3. Каждый вызов `dbmerge` — `data_types={'merged_on': DateTime(timezone=True), 'inserted_on': …}`,
   иначе новая таблица снова родится наивной. Параметр уже есть и перекрывает тип и для
   автоматически ведомых колонок — проверено на живой БД, правки в dbmerge не нужны.
4. Примеры витрин в `config/handlers/`: в их DDL `merged_on timestamp` прописан руками.
5. Наивные константы в сравнениях (`EPOCH` в цикле обработчиков и подобные) становятся aware.
6. README_DB (контракт `merged_on`), DESIGN, CHANGELOG.

**Что ломается.** Переезд односторонний: старая версия, прочитав `timestamptz`, получит aware-
значения и упадёт на сравнениях с наивными. В CHANGELOG — в Breaking, с указанием обратного
`ALTER … TYPE timestamp` на случай отката.

**Порядок работ:**

1. Миграция и сверка типов (`db_logs`), тесты на них — сначала механизм, потом переключение.
2. Объявления колонок + `DB_NOW_WITH_TIMEZONE` + `data_types` в вызовах dbmerge.
3. Прогон тестов: сравнения naive/aware вылезут именно здесь, это и есть основной объём.
4. Примеры витрин и документация.
5. Живой прогон на демо УТ: миграция действующей схемы `onecdc_trade_demo`, затем обычный цикл.
